...
每次执行此命令都只打开网页。选择已保存的机器人配置，点击“开启机器人”后才启动厂商驱动、平台运行时和所选设备；已有机器人或显式传入 robot_id 也不会自动开启。
驱动的启动步骤直接使用机器人已保存的配置，不需要另一套启动配置。命令行的遥操作/相机选项只设置网页按钮的初始选择。
网页提供“开启机器人 / 关闭机器人 / 重启机器人”，关闭与重启只影响本入口管理的机器人服务，网页保持运行。
同一插件目录通过进程锁限制为一个管理入口和一套机器人运行进程。锁冲突显示占用 PID，退出后自动释放。
Linux 下入口或网页进程异常退出时，受控子进程收到正常退出信号，避免独立会话留在后台继续运行。
...
通过 `vendor_*` 参数配置的厂商 launch 由独立子进程运行并受统一入口监管。
厂商延迟启动的控制器保留自己的 launch 参数上下文；关闭或重启机器人会处理该子进程，
厂商 launch 退出时平台也会关闭。厂商 launch 应自行监管其关键节点的异常退出。
...
- **夹爪插件与测试**：夹爪页自动列出已导入的 `gripper_driver` 插件，可直接导入、切换或移除。当前插件按逻辑夹爪提供“测试打开 / 测试闭合”按钮，通过统一 JointState 命令与反馈话题确认动作；厂商话题和换算保留在插件包中。发送目标前检查对应夹爪运行时的新鲜驱动诊断；ROS 话题插件报告底层命令没有订阅者时拒绝测试。关节反馈正常、控制器名称相同或处于 active 状态，都不能证明控制器接口与插件匹配。
...
- **相机拍照检查**：保存配置并启动相机后，拍照最多等待 10 秒获取一帧新鲜彩色图像。启动入口传入 `start_cameras:=false` 时，网页“启动相机”默认不勾选；需要在网页勾选，再点击“开启/重启机器人”，才会启动已保存且启用的相机。拍照按钮本身只接收图像。按发布端能力选择可靠接收，兼容仅提供 BEST_EFFORT 的图像流；不读取历史缓存照片。失败时区分未发现发布端、消息类型错误、ROS 丢消息及图像时间戳异常，附带实际话题、ROS domain_id 和接收 QoS。
...
运行控制测试覆盖新安装及已有机器人都等待点击开启、上次选择持久化、重复启动互斥、端口占用不启动机器人、
