...
底盘和夹爪的网页配置位于“机器人配置”的对应页签。夹爪页用于发现、导入、切换和移除 `gripper_driver` 插件，并可在当前版本运行后测试打开/闭合；厂商话题、单位、方向和限位随插件包保存。使能按键、输入轴、开合位置和速度等遥操作映射保存在 `hc_teleop_config`。接口默认关闭，可配置单夹爪、双夹爪或更多独立夹爪。网页保存不会启动机器人。
