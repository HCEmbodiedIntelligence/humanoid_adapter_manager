...
到位按容差判断，无需实测值与目标严格相等。米制夹爪默认闭合容差为 `0.002 m`（2 mm），
打开及弧度制夹爪保留 `0.001` 的默认值；默认容差同时限制为开合行程的 5%，适配小行程夹爪。
可在对应夹爪的能力字段中设置 `position_tolerance`，或分别设置
`open_position_tolerance` / `closed_position_tolerance`；单方向设置优先。
容差使用该夹爪的逻辑位置单位，必须为正数且小于开合行程的一半。
驱动诊断正常且新鲜实测反馈进入容差范围，就报告到位；无需额外连续保持 0.2 秒。
先判断最新反馈是否到位，再判断超时，避免截止时刻已到位仍被报错。
测试发起运动后，到位时请求保持实测位置；已在容差内则直接成功，不再发送运动指令。
反馈过期、驱动异常或超时后仍超出容差时，测试仍会失败。
