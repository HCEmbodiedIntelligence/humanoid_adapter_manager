...
robot_id:'机器人 ID',buttons_topic:'按钮事件话题',adapter:'管理器关联',
  ...
  function showImportDialog(kind='workspace',expected='',title='导入配置包'){const form=$('#managedImportForm');form.reset();$('#managedImportKind').value=kind;$('#managedImportExpectedType').value=expected;$('#managedImportTitle').textContent=title;$('#managedImportIdentity').classList.toggle('hidden',kind!=='workspace');$('#managedImportError').textContent='';$('#managedImportDialog').showModal();}
