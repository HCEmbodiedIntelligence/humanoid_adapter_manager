...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.locator('#launcherTeleop').isChecked(),false);
    assert.equal(await page.locator('#launcherCameras').isChecked(),false);
    assert.equal(await page.locator('#launcherSettings').count(),0,'Reuse robot configuration without a second startup form');
    ...
    await page.reload();
    await page.waitForFunction(()=>!document.querySelector('#startManagedRobot').disabled);
    assert.equal(mutations.length,beforeReload,'Reloading must not start the saved robot');
    state=await (await page.request.get(base+'api/launcher')).json();
    assert.equal(state.runtime.phase,'stopped');
